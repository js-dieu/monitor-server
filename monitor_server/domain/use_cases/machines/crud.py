...
class AddMachine(UseCase[CreateMachine, NewMachine]):
    def __init__(self, machine_repository: ExecutionContextRepository) -> None:
        self._repository = machine_repository

    def execute(self, input_dto: CreateMachine) -> UseCaseResult[NewMachine]:
        try:
            machine = t.cast(Machine, Machine.from_dict(input_dto.to_dict()))
            self._repository.create(machine)
            return UseCaseResult(status=True, data=NewMachine(uid=machine.uid))
        except ORMError as e:
            return UseCaseResult(status=False, msg=str(e), data=NewMachine())
