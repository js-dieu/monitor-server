...
class SessionSQLRepository(SQLRepository[MonitorSession, Session]):
    def create(self, item: MonitorSession) -> MonitorSession:
        ...
        try:
            ...
            raise EntityAlreadyExists(MonitorSession, item.uid.hex) from e
    ...
    def list(self, page_info: PageableStatement | None = None) -> PaginatedResponse[t.List[MonitorSession]]:
        ...
        if page_info:
            ...
            return page_info.build_response(
                [self.mapper.cast_model(row, MonitorSession) for row in rows or []], elements_count=count
            )

        rows = t.cast(t.Iterable[Session], q.all())
        return PaginatedResponse(
            data=[self.mapper.cast_model(row, MonitorSession) for row in rows or []],
            page_no=None,
            next_page=None,
        )
...
class SessionInMemRepository(InMemoryRepository[MonitorSession, Session]):
    def get(self, uid: str) -> MonitorSession:
        ...
        if row is None:
            raise EntityNotFound(MonitorSession, uid)
    ...
    def create(self, item: MonitorSession) -> MonitorSession:
        if item.uid.hex in self._data:
            raise EntityAlreadyExists(MonitorSession, item.uid.hex)
    ...
    def update(self, item: MonitorSession) -> MonitorSession:
        if item.uid.hex not in self._data:
            raise EntityNotFound(MonitorSession, item.uid.hex)
        self._data[item.uid.hex] = Session(
            uid=item.uid, description=item.tags, run_date=item.start_date, scm_id=item.scm_revision
        )
        return item
    ...
    def list(self, page_info: PageableStatement | None = None) -> PaginatedResponse[t.List[MonitorSession]]:
        if page_info is None:
            return PaginatedResponse(
                data=[
                    self.mapper.cast_model(session, as_=MonitorSession)
        ...
        return page_info.build_response(
            data=[self.mapper.cast_model(self._data[element_id], as_=MonitorSession) for element_id in element_ids],
